/* ===== Speed engine (pure) — Pokémon Champions Regulation M-C =====
...
const Engine = (() => {
  ...
  const ITEMS = { none: 1, scarf: 1.5, ironball: 0.5 };
  const ITEM_LABEL = { none: '—', scarf: 'Choice Scarf', ironball: 'Iron Ball' };
  ...
  function activeAbility(entity, spec, field, ABILITIES) {
    ...
    for (const slug of entity.abil || []) {
      ...
      if (spec.abil === 'on' || spec.abil === slug) return { slug, mult: a.mult };
      if (spec.abil !== 'auto') continue;
      if (slug === 'quick-feet' && spec.para) return { slug, mult: a.mult };
  ...
  function compute(entity, spec, field, ABILITIES) {
    ...
    const im = entity.mega ? 1 : ITEMS[spec.item] ?? 1;
    if (entity.mega && spec.item !== 'none') steps.push({ label: 'Mega Stone required; other held-item modifiers do not apply', value: v });
    ...
    if (spec.para && ab?.slug !== 'quick-feet') { v = Math.floor(v * 0.5 + 1e-9); steps.push({ label: 'Paralysis ×0.5', value: v }); }
    else if (spec.para) steps.push({ label: 'Quick Feet ignores paralysis penalty', value: v });
  ...
  function solve(entity, spec, field, ABILITIES, target, trickRoom) {
    const targetPriority = typeof target === 'object' ? target.priority : 0;
    if (spec.priority > targetPriority) return [{ label: 'already moves first (higher priority)', ok: true }];
    if (spec.priority < targetPriority) return [{ label: 'target has higher priority; Speed changes cannot move first', blocked: true }];
    target = typeof target === 'object' ? target.speed : target;
    ...
    if (!trickRoom) {
      ...
      if (!entity.mega && spec.item !== 'scarf') { const s = compute(entity, { ...spec, item: 'scarf' }, field, ABILITIES).speed; if (beats(s)) out.push({ label: `Choice Scarf (${s})` }); }
      for (let st = spec.stage + 1; st <= 6; st++) { const s = compute(entity, { ...spec, stage: st }, field, ABILITIES).speed; if (beats(s)) { out.push({ label: `Stage ${st > 0 ? '+' : ''}${st} (${s})` }); break; } }
      ...
      if (!entity.mega && spec.item !== 'ironball') { const s = compute(entity, { ...spec, item: 'ironball' }, field, ABILITIES).speed; if (beats(s)) out.push({ label: `Iron Ball (${s})` }); }

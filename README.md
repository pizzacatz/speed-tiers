...
| Anchor dock | Pin any row(s) (◆). The dock sits directly under the header and stays pinned to the top while you scroll; edit the active anchor's set live. Every row shows **outspeeds / speed tie / slower ±N** relative to the anchor at the sort column, a divider marks the anchor's position, and hovering a row shows what the anchor needs to beat it (SP threshold, +Spe alignment, Scarf, stage, Tailwind), and a summary counts how many visible rows are faster / tied / slower. Pin your whole team and switch between chips. |

...
| Columns | Preset "what-if" specs shown side by side: **Min** (0 SP, −Spe), **Base** (0 SP, neutral — the in-game presented stat), **Max** (32 SP, +Spe), **Scarf**, **+1**, **−1**, **Tailwind**, **Scarf+TW**. Click a column to sort by it; hide, drag-reorder, edit, or add your own (e.g. *Max in Rain*). |
...
| Anchor dock | Pin any row(s) (◆). The dock stays fixed at the bottom while you scroll; edit the active anchor's set live. Every row shows **outspeeds / speed tie / slower ±N** relative to the anchor at the sort column, a divider marks the anchor's position, and hovering a row shows what the anchor needs to beat it (SP threshold, +Spe alignment, Scarf, stage, Tailwind). Pin your whole team and switch between chips. |
